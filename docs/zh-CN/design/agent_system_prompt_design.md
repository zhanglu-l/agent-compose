...
| Capabilities (MPI) | Host 从 OctoBus capset 写入 `runtime/mpi/catalog.md` | 仅注入 Codex / Claude |
...
- 当两层均存在时，Agent Identity 排在 Capabilities (MPI) **之前**
...
- 无可用 MPI catalog 时，省略 Capabilities (MPI)。
- 仅存在 MPI 时，runtime 保留 Phase 1 之前的 MPI 字符串（含 `formatMpiContext` 生成的 `## MPI Catalog` 标题）。
...
当 system prompt 非空时，guest 命令形态：
...
当 `system_prompt` 为空时，完全省略 `--system-prompt-file`。
...
**函数：** `Executor.resolveAgentSystemPrompt(ctx, session, providerAgent string) (string, error)`

解析顺序：

1. 若 `providerAgent`（即 `ExecuteAgentRequest.AgentDefinitionID`）非空，直接加载该 agent definition。
...
**函数：** `writeAgentSystemPromptFile(config, session, provider, systemPrompt string) (string, error)`

| 属性 | 值 |
| --- | --- |
| Host 目录 | `{hostSessionDir}/state/agents/system-prompts/` |
| 文件名 | `{normalizeAgentKind(provider)}-{unix_nano}.txt` |
| Guest 路径 | `{GuestStateRoot}/agents/system-prompts/{同名}` |
| 内容 | UTF-8 原始 `systemPrompt` 字节（section 标题由 guest runtime 添加） |
| 空 prompt | 返回 `""`；不创建文件 |

实现方式镜像 per-turn 消息的 `writeAgentPromptFile`。
...
`buildAgentExecSpec` 接受可选的 `systemPromptPath`，仅非空时追加 `--system-prompt-file`。

### 向后兼容重试

若 guest runtime 不识别 `--system-prompt-file`（stderr 含该 flag 的 `unknown option` 或 `unknown flag`），host 记录 warning 并在不带该 flag 的情况下重试一次。这样在 runtime JS 升级前，旧 guest 镜像仍可运行（MPI-only）。
...
| 写入 system prompt 文件失败 | 失败（与 prompt 文件写入失败相同） |
| 空 system prompt | 省略 `--system-prompt-file` |
| 旧 guest runtime | 不带 system prompt 文件重试 |
...
- agent prompt 与 MPI 均非空：追加 `## Capabilities (MPI)`、空行、trim 后的 MPI 正文。
- agent prompt 为空但 MPI 存在：原样返回 MPI（保留 `formatMpiContext` 的 `## MPI Catalog` 标题，向后兼容）。
- 两者均为空：返回 `""`。
...
- `cli.ts`：`prompt` 子命令新增可选 `--system-prompt-file <path>`。
- `prompt.ts`：读取文件、读取 MPI catalog、调用 `buildSystemContext`，将结果作为 `systemContext` 传给 runner。
...
Phase 1 中 Gemini runner 的额外调整：

- `--skip-trust` CLI flag
- 子进程环境变量 `GEMINI_CLI_TRUST_WORKSPACE=true`

这些用于减少自动化运行中的交互式 trust 提示，与 prompt 分层无关，但在同一变更集中落地。
...
- `writeAgentSystemPromptFile` 映射 host → guest 路径并写入内容
- 空 prompt 跳过文件创建
- `buildAgentExecSpec` 含/不含 `--system-prompt-file`
...
| 文件 | 变更 |
| --- | --- |
| `pkg/agentcompose/service.go` | `resolveAgentSystemPrompt`、`writeAgentSystemPromptFile`、`executeAgentRun`、`buildAgentExecSpec`、兼容重试 |
...
| `pkg/agentcompose/agent_system_prompt_test.go` | **新增** — host 解析、文件写入、exec spec 测试 |
| `runtime/javascript/src/system-context.ts` | **新增** — 组合与文件读取辅助函数 |
| `runtime/javascript/src/cli.ts` | `--system-prompt-file` 选项 |
| `runtime/javascript/src/prompt.ts` | runner 分发前组合 `systemContext` |
...
| `docs/design/agent-compose-runtime-js_contract.md` | 记录新 flag 与分层 |
| `openspec/specs/agent-system-prompt/spec.md` | 规范要求 |
...
统一 MPI section 标签。Phase 1 保留 `formatMpiContext` 输出为 `## MPI Catalog`；当 agent identity 与 MPI 同时存在时，`buildSystemContext` 会追加外层 `## Capabilities (MPI)` 标题，可能产生嵌套标题。后续应剥离或重命名内层标题，使所有场景标签一致。
