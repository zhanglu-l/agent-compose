...
8. 在同一文件系统内**原子 rename** qcow2 和 manifest，最后发布 ready 状态。
...
- **已发布的母盘不得被重复发布覆盖。** 发布前需确认目标路径不存在；已存在即视为其他执行者已完成构建，直接复用。若允许覆盖，一个已被子盘引用的母盘会被换成新 inode，导致既有子盘的 backing 指向失效。
...
| 母盘发布与子盘发布必须原子；ready manifest 不得先于对应磁盘出现 | 其他进程读到半成品并当作可用 |
