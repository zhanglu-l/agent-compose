...
- 现有 `task test:e2e:docker-jupyter` 保留为聚焦诊断入口，并复用完整 harness 中的 Jupyter lifecycle 场景。
...
| `LLM-003` | runtime facade 支持 OpenAI responses/chat 和 Anthropic messages，模型与 wire scope 正确 | 三 driver |
